...
\newsection{Comparison to Existing Implementation} \label{Section_Comparison}
	...
	Briefly put, Rogue Reborn is very much reminiscent of its classical ancestor.  This successful correlation can be attributed to the satisfaction of the \hyperref[Section_FR_Evaluation]{functional }and \hyperref[Section_NFR_Evaluation]{ non-functional requirements } set forth at the commencement of the project: the majority of these requirements were directly influenced by features present in the original \textit{Rogue}.  Through the act of observing key features of the existing implementation and expressing them as functional requirements, the Rogue++ team was able to define a very precise target for functionality.  There were also several non-functional requirements (primarily from the aesthetics and usability sections) that were inspired by the original artwork and gameplay. \\

	Naturally, the projects also diverged in various aspects.  The Rogue Reborn implementation introduced several new features to the game that the development team believed would improve its entertainment value.  One example of such an improvement is the GUI colour: the original game was monotone, which by today's standards would be considered tame, mundane, boring, and even lazy.  As such, the Rogue++ team decided that using a more varied colour palette would better reflect the life, flavour, and vibrant creativity portrayed through the game.  Virtually all expected improvements with respect to \textit{Rogue} were modelled as non-functional requirements since the core functionality was preserved across the implementations.\\

	The combination of requirements derived from the original \textit{Rogue} and the faithful improvements implemented by the Rogue++ developers resulted in an application that is functionally equivalent to the original game yet non-functionally superior in almost every way.   In other words, the Rogue++ team breathed modern life into an otherwise ancient game. The project truly is \textit{Rogue - Reborn}.
