...
\newsection{Non-Functional Requirements Evaluation} \label{Section_NFR_Evaluation}
	...
	\subsection{Performance}
		...
		A synopsis of the relevant tests (\nfrt{5} and \nfrt{6}) is portrayed below.

		\nfrtc {5}
			{The Rogue Reborn application was compiled with a special debug parameter; this enabled particular sections of the MasterController module to use a StopWatch implementation to measure the average maximum execution time between successive frames.}
			{On average, the execution time between successive user actions appeared to stabilize around 20 ms.  Clearly, this is appreciably lower than the maximum allowable delay of $\sim$ 30 ms.}
		...
		\nfrtc {6}
			{The following regular expression was applied over the source code to extract all integer-typed declarations: \newline
			 ...
			 An associated script to perform the declaration extraction was also created and is located in the \lstinline$src/misc$ directory under the name \href{run:../../src/misc/intdeclare.py}{intdeclare.py}.}
